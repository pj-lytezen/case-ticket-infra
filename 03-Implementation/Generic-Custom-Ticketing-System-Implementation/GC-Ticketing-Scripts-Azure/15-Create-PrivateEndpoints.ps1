...
$peSubnetId = (Invoke-Az "network vnet subnet show -g $rg --vnet-name $vnetName -n $peSubnetName --query id -o tsv").Trim()
...
function Ensure-PrivateDnsZoneAndLink {
  ...
  try { Invoke-Az "network private-dns zone show -g $rg -n $ZoneName" | Out-Null } catch {
    Write-Host "Creating Private DNS zone: $ZoneName"
    Invoke-Az "network private-dns zone create -g $rg -n $ZoneName" | Out-Null
  }
  try { Invoke-Az "network private-dns link vnet show -g $rg -z $ZoneName -n $LinkName" | Out-Null } catch {
    Write-Host "Linking DNS zone to VNet: $ZoneName"
    Invoke-Az "network private-dns link vnet create -g $rg -z $ZoneName -n $LinkName -v $vnetId -e false" | Out-Null
  }
}
...
function Ensure-PrivateEndpoint {
  ...
  try {
    Invoke-Az "network private-endpoint show -g $rg -n $PeName" | Out-Null
  ...
  $peCmd = @(
    "network private-endpoint create -g $rg -n $PeName -l $Location",
    "--subnet $peSubnetId",
    "--private-connection-resource-id $TargetResourceId",
    "--group-id $GroupId",
    "--connection-name $PeName"
  ) -join " "
  Invoke-Az $peCmd | Out-Null

  # DNS zone group to auto-register DNS records.
  $zoneGroupName = "zg-$PeName"
  try {
    Invoke-Az "network private-endpoint dns-zone-group show -g $rg --endpoint-name $PeName -n $zoneGroupName" | Out-Null
  } catch {
    Write-Host "Creating DNS zone group for $PeName"
    $zoneId = (Invoke-Az "network private-dns zone show -g $rg -n $DnsZoneName --query id -o tsv").Trim()
    $zoneCmd = @(
      "network private-endpoint dns-zone-group create -g $rg --endpoint-name $PeName -n $zoneGroupName",
      "--private-dns-zone $zoneId --zone-name $DnsZoneName"
    ) -join " "
    Invoke-Az $zoneCmd | Out-Null
  }
}
...
$stId = (Invoke-Az "storage account show -g $rg -n $storageName --query id -o tsv").Trim()
...
$kvId = (Invoke-Az "keyvault show -g $rg -n $kvName --query id -o tsv").Trim()
